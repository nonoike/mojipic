...
class PicturePropertyRepositoryImpl extends PicturePropertyRepository {
  ...
  override def find(pictureId: PictureId): Future[PictureProperty] = {
    Future.fromTry(Try {
      using(DB(ConnectionPool.borrow())) { db =>
        db.readOnly { implicit session =>
          val sql =
            sql"""SELECT
              ...
              """.stripMargin
          sql.map(resultSetToPictureProperty).single().apply().getOrElse(throw new RuntimeException(s"Picture is notfound. PictureId: ${pictureId.value}"))
        }
      }
    })
  }
